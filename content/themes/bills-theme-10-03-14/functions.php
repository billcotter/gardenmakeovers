...
define( 'CHILD_THEME_NAME', __( 'Bills Theme 10-03-14', 'bills_theme' ) );
define( 'CHILD_THEME_URL', 'http://billjcotter.com' );
...
//* Enqueue scripts and styles wp_enqueue_style( 'google-fonts', '//fonts.googleapis.com/css?family=Lora:400,700|Oswald:400', array(), PARENT_THEME_VERSION );
...
add_action('genesis_before_loop', 'bjc_custom_content');
function bjc_custom_content() {
if ( is_singular(array('post', 'page')) && genesis_get_custom_field('messgae_field') )
echo '<div class="custom-content">'. genesis_get_custom_field('messgae_field') .'</div>';
}
*/

add_action('genesis_before_loop', 'bjc_custom_content');
function bjc_custom_content() {
if ( is_singular(array('post','page')) && genesis_get_custom_field('message_field') )
echo '<div class="content-box-blue">'. genesis_get_custom_field('message_field') .'</div>';
} 
