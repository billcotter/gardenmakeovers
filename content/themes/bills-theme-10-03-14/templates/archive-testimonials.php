...
function custom_do_grid_loop() {  
	...
	if( $loop->have_posts() ):
		...
		while( $loop->have_posts() ): $loop->the_post(); global $post;
			...
			echo '<h1><h1><?php the_field('client_name'); ?></h1>';

			echo '<h1><?php the_field('client_title'); ?></h1>';
			echo '<div id="testimonials">';
			echo '<div class="one-fourth first">';
			echo '<div class="quote-obtuse"><div class="pic">'. get_the_post_thumbnail( $id, array(150,150) ).'</div></div>';
			echo '<div style="margin-top:20px;line-height:20px;text-align:right;"><cite>'.genesis_get_custom_field( 'client_name' ).'</cite><br />'.genesis_get_custom_field( 'client_title' ).'</div>';
			echo '</div>';	
